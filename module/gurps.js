...
`
  ...
  GURPS.addUse = function (event) {
    ...
    if (game.user.isGM) {
      ...
      const totalUses = skillUses + Number(uses)
      if (skill) {
        const json = `{ "system.skills.${skill.parentKey}.uses": ${totalUses} }`
        actor.update(JSON.parse(json))
        $(event.target).remove()
  ...
  async function executeOTF(inputstring, priv = false, event = null, actor = null) {
    ...
    for (let string of strings) {
      ...
      console.log(action.action.type, action.action.name, actor || GURPS.LastActor)
      answer = false
      if (!!action.action) {
  ...
  function findSkillSpell(actor, sname, isSkillOnly = false, isSpellOnly = false) {
    ...
    let best = 0
    if (!isSpellOnly)
      recurselist(actor.skills, function (s, parentKey) {
    ...
    if (!t)
      if (!isSkillOnly)
        recurselist(actor.spells, function (s, parentKey) {
          if (s.name?.match(skillRegExp) && s.level > best) {
            s.parentKey = parentKey
            t = s
            best = parseInt(s.level)
  ...
  Hooks.once('ready', async function () {
    ...
    $('body').on('click', '.addUse', GURPS.addUse)
