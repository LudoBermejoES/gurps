...
export class GURPSRange {
  ...
  static get penaltiesPerFiveRanges() {
    const penaltiesPerFiveRanges = []
    for (let i = 0; i < 50; i++) {
      penaltiesPerFiveRanges.push({
        moddesc: i18n_f('GURPS.modifierRange', { range: (i + 1) * 5 }),
        max: (i + 1) * 5,
        penalty: -i,
        desc: `${(i + 1) * 5} yds`,
      })
    }
    return penaltiesPerFiveRanges
  ...
  async update() {
    ...
    switch (currentValue) {
      ...
      case 'FivePenalties': {
        this.ranges = GURPSRange.penaltiesPerFiveRanges
        break
      }
      case 'TenPenalties': {
